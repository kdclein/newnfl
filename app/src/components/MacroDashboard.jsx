...
function RecessionModels({ regime }) {
  const ind = regime?.indicators || {};
  const probs = ind.wells_ordered;
  const inputs = ind.wells_inputs;
  const model = ind.recession_model;
  const rec = regime?.recession_probability != null ? Number(regime.recession_probability) : null;
  if (rec == null && !probs) return null;
  const recPct = rec == null ? 0 : Math.round(rec * 100);
  const top = probs && REGIMES.reduce((a, b) => ((probs[b.id] ?? 0) > (probs[a.id] ?? 0) ? b : a), REGIMES[0]);

  return (
    <section className="card p-4 mt-5">
      <h3 className="text-xs uppercase tracking-wider font-semibold text-white/60">
        📉 Recession &amp; Regime Models
      </h3>

      {/* Model 1 — single-probability yield-curve probit */}
      {rec != null && (
        <div className="mt-3">
          <div className="flex items-baseline justify-between gap-3 flex-wrap">
            <span className="text-[11px] font-semibold text-white/70">Yield-Curve Probit</span>
            <span className="text-[10px] text-white/40">NY Fed · Estrella–Mishkin · 12-mo</span>
          </div>
          <div className="mt-2">
            <div className="flex items-baseline justify-between gap-2">
              <span className="text-[12px] font-medium text-white/85">Recession probability</span>
              <span className="font-mono tabular text-[12px] font-semibold" style={{ color: "#f87171" }}>
                {recPct}%
              </span>
            </div>
            <div className="mt-1 h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
              <div className="h-full rounded-full" style={{ width: recPct + "%", background: "#f87171" }} />
            </div>
          </div>
          {model && (
            <div className="mt-2 text-[10px] text-white/40 font-mono flex flex-wrap gap-x-3 gap-y-0.5">
              {model.probit_10y3m != null && <span>10Y−3M probit {Math.round(model.probit_10y3m * 100)}%</span>}
              {model.sahm_gap != null && <span>Sahm +{model.sahm_gap}</span>}
              {model.hy_oas != null && <span>HY {model.hy_oas}pp</span>}
            </div>
          )}
          <p className="text-white/30 text-[10px] mt-1.5 leading-snug">
            The classic single-outcome model: a probit on the 10Y−3M term spread, floored by the Sahm rule and
            nudged by high-yield credit stress. Answers one question — recession in the next year, yes or no.
          </p>
        </div>
      )}

      {/* Model 2 — Wells Fargo-style ordered probit */}
      {probs && (
        <div className="mt-4 pt-4 border-t border-white/10">
          <div className="flex items-baseline justify-between gap-3 flex-wrap">
            <span className="text-[11px] font-semibold text-white/70">One-Year-Ahead Regime Outlook</span>
            <span className="text-[10px] text-white/40">
              Wells Fargo–style · base case:{" "}
              <span style={{ color: top.color }} className="font-semibold">{top.label}</span>
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[11px] text-white/45 font-mono">
                ...
                <span className="text-white/30"> · see models below</span>
              </div>
            </div>
          </div>
